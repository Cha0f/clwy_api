...
- 注册成功邮件通知（RabbitMQ 异步队列）
- 基于 Winston 的日志系统，通过 RabbitMQ 异步持久化到 MySQL
- 全局未捕获异常与未处理 Promise 拒绝的日志兜底
...
│   ├── like.js                    # 用户点赞关系模型
│   ├── log.js                     # 日志模型（与 winston + RabbitMQ 异步持久化配合）
│   ├── setting.js                 # 单例系统设置模型
...
│   ├── log-consumer.js            # RabbitMQ 日志队列消费者（异步写入 Logs 表）
│   ├── logger.js                  # Winston 日志实例（Console + RabbitMQ 传输器）
│   ├── logger-transport.js        # 自定义 RabbitMQ Winston Transport
│   ├── mail.js                    # Nodemailer 发件箱配置与邮件发送
│   ├── pagination.js              # 安全分页参数解析
│   ├── rabbit-mq.js               # RabbitMQ 连接、邮件队列生产与消费
...
| `utils/mail.js`               | Nodemailer 发件箱配置与邮件发送                                          |
| `utils/rabbit-mq.js`          | RabbitMQ 连接管理、邮件队列生产与消费                                   |
| `utils/logger.js`             | Winston 日志实例（Console 传输器 + RabbitMQ 异步传输器）                |
| `utils/logger-transport.js`   | 自定义 RabbitMQ Winston Transport，将日志消息投递到 RabbitMQ 队列       |
| `utils/log-consumer.js`       | 从 RabbitMQ 日志队列消费，通过 Sequelize 写入 Logs 表                   |
...
可以使用项目提供的 Docker Compose 启动 MySQL、Redis 和 RabbitMQ：
...
| `MAILER_HOST`           | 发件必填 | 无                       | SMTP 服务器地址                       |
| `MAILER_PORT`           | 否       | `587`                    | SMTP 服务器端口                       |
| `MAILER_SECURE`         | 否       | `false`                  | 465 端口填写 `true`，否则填写 `false` |
| `MAILER_USER`           | 发件必填 | 无                       | 邮箱地址                              |
| `MAILER_PASS`           | 发件必填 | 无                       | 邮箱授权码或密码                      |
| `RABBITMQ_URL`          | 否       | `amqp://localhost:5672`   | RabbitMQ 连接地址                    |
...
- Express、Sequelize CLI、JWT、Redis、COS、邮件和消息队列共用同一配置入口。
...
| 方法   | 路径                    | 说明               |
| ------ | ----------------------- | ------------------ |
| GET    | `/admin/logs`           | 日志列表（倒序）   |
| GET    | `/admin/logs/:id`       | 日志详情           |
| DELETE | `/admin/logs/clear`     | 清空全部日志       |
| DELETE | `/admin/logs/:id`       | 删除单条日志       |

日志通过 Winston + RabbitMQ 异步队列写入 `Logs` 表，`GET /admin/logs` 按 `id` 倒序返回最近的日志记录。

## 日志架构

应用使用 Winston 日志系统，日志消息通过自定义 `RabbitMQTransport` 投递到 RabbitMQ `log_queue`，由独立的 `logConsumer` 异步消费并通过 Sequelize 写入 `Logs` 表。

```
logger.error/info/warn()
      │
      ├── Console Transport          ← 始终输出，方便开发调试
      │
      └── RabbitMQTransport          ← 异步投递，不阻塞主线程
              │
              ▼  RabbitMQ log_queue (持久化、maxLength 10000)
              │
         logConsumer
              │
              ▼  Log.create() → Logs 表
...
- **生产端容错**：RabbitMQ 不可用时，Transport 回退到 `console.error`，不崩溃进程。
- **消费者容错**：使用 `noAck: false` + 手动 ack，MySQL 瞬断时消息重新入队重试。
- **防积压**：队列设置 `maxLength: 10000`，消费者 `prefetch(10)`。
- **全局兜底**：`process.on('unhandledRejection')` 和 `process.on('uncaughtException')` 记录所有未捕获异常。
...
- 生产 RabbitMQ 应启用访问控制、TLS 并限制网络访问。
